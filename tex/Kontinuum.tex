\chapter{Materialno telo} \label{chp:kinkon}
...
tj.~za vsak $\varepsilon>0$ obstaja $\delta>0$, tako da za vsaka $t_1,t_2\in I$,
...
ki je potemtakem tudi zvezno, enakega predznaka povsod na $\B\times I$.
...
\begin{gather*}
	\langle\Grad\phi,\vek{w}\rangle=\langle\grad\phi,(\Grad\vek{\chi})\vek{w}\rangle=
...
del robu $\partial\B$, potem lahko dolžinska elementa $d\vek{x}_1$, $d\vek{x}_2$,
...
\end{align*}
Pri tem smo člene, kjer nastopajo potence števila $\varepsilon$, višje od 1, spravili
v izraz $o(\varepsilon)$. Od tu lahko sedaj izračunamo smerni odvod
\begin{multline*}
...
(kjer vzamemo $\W=\V$), definicijo skalarnega produkta na prostoru $\L(\V)$ ter zvezo (\ref{e:L}).
...
\begin{equation} \label{e:rojror}
	\rho_R(X)=\rho(X,t)J(X,t)\qquad\textrm{za vse}\ X\in \B,\ t\in I.
\end{equation}
